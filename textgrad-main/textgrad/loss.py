...
class PreOptTestTime(Module):
    def __init__(self,
                 evaluation_instruction: str,
                 engine: Union[EngineLM, str] = None,
                 system_prompt: Variable = None):
        """
        The test-time loss to use when working on preference optimization.
        ...
        if system_prompt:
            ...
            tt_system_prompt = """You are a language model that evaluates the quality of responses in a general instruction-following task. Assess each response based on the following principles:

1. **Helpfulness:** The response should directly address the instruction, providing relevant and thorough information to meet the user's needs. It should avoid unnecessary or unrelated content.  

2. **Harmlessness:** The response must avoid causing harm, offense, or promoting dangerous, unethical, or biased information. It should respect user safety and societal norms.  

3. **Honesty:** The response should provide accurate and truthful information. If a confident answer is not possible, the response should acknowledge its limitations rather than fabricate information.  

Do not try to solve the problem, only raise the potential issues and mistakes in the answer. Be creative, think about different perspectives, and be very critical."""
            
            # "You are an intelligent assistant used as an evaluator, and part of an optimization system. You will analyze a solution to a model response of a user query. Investigate the reasoning and answer. Do not try to solve the problem, only raise the potential issues and mistakes in the answer. Be creative, think about different perspectives, and be very critical."
            self.tt_system_prompt = Variable(tt_system_prompt,
                                                requires_grad=False,
                                                role_description="system prompt for the test-time evaluation")
    ...
    def forward(self, question: str, prediction: Variable) -> Variable:
        question_variable = Variable(question, 
                                     requires_grad=False, 
                                     role_description="the user prompt")
