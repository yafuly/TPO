            ...
            if len(new_text) == 1:
                ...
                for _text in new_text:
                    try:
                        ...
                        continue
                # # if all fails, at least keep one (full) text for the next iteration
                # if len(extracted_texts) == 0:
                #     print("Warning: none valid improved variables, use unprocessed step variables ...".upper())
                #     print(new_text[0])
                #     extracted_texts.append(new_text[0])
...
class TextualGradientDescentWithRM(Optimizer):
    def __init__(self, 
                 parameters: List[Variable], 
                 verbose: int=1, 
                 engine: Union[EngineLM, str]=None, 
    ...
    def step(self, query, reward_model, index=0, n=1):
        """
        Perform a single optimization step.
        This method updates the parameters of the optimizer by generating new text using the engine and updating the parameter values accordingly.
        It also logs the optimizer response and the updated text.
        ...
        assert len(self.parameters) == 1 # multi-param TBD
        for parameter in self.parameters:
            print("parameter".upper())
            print(parameter)
            prompt_update_parameter = self._update_prompt(parameter)
            print("prompt_update_parameter".upper())
            print(prompt_update_parameter)
            new_texts = self.engine(prompt_update_parameter, system_prompt=self.optimizer_system_prompt, **diverse_gen_params)

            assert n>1 # n=1 TBD
            if n>1:
                print("new_texts".upper())
                print(new_texts)
                
                extracted_responses = [text.split(self.new_variable_tags[0])[1].split(self.new_variable_tags[1])[0].strip() for text in new_texts]
                sample_qas = [(query,res) for res in extracted_responses]
                scores = reward_model.perform_rm(sample_qas)
                print("#"*100)
                print(len(scores))
                print(len(reward_model.score_cache))
                reward_model.cache_scores(scores, sample_qas, index=index)
                print(len(reward_model.score_cache))
                contrastive_responses = reward_model.get_contrastive_samples(scores, sample_qas)
                contrastive_prompt = reward_model.prompt_template.format(
                    query=query,
                    bad_response=contrastive_responses['worst'],
                    good_response = contrastive_responses['best']
                )
                solution_text = self.engine(contrastive_prompt, **determin_gen_params)
                solution = contrastive_prompt + f"\n\nModel response: {solution_text}"
                parameter.set_value(solution)
